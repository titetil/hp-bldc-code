...
*
* $Workfile: shl.h $ $Revision: 22 $ $Date: 28.10.03 15:21 $ $Author: Dagmara $
******************************************************************************/
...
#define C__I64SHLI64C6_LT32(hi, lo, csh, csh32)\
    hi = (Int32)( (UInt32)(hi << csh) + (UInt32)(lo >> csh32) );\
    lo = (UInt32)(lo << csh)
...
#define C__I64SHLI64U6(hi, lo, sh)\
    ...
    {\
        if(sh < 32)\
        {\
            hi = (Int32)( ( (UInt32)(hi) << sh) + (lo >> (32-sh)) );\
            lo <<= sh;\
        }\
        else if (sh == 32)\
        {\
